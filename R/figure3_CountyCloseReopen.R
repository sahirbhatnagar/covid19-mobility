...
figure3_CountyCloseReopen_fun <- function(res, input.params, today = "2020-12-08"){
  ...
  DT_present <- filter(DT_merged, 
                       
                       (date == today & state == "Ohio")
                       
                       | (date == today & state == "Missouri")
                       
                       | (date == today & state == "South Carolina") 
                       
                       | (date == today & state == "Indiana"))
  ...
  DT_both$state = factor(DT_both$state, 
                         levels=c("Ohio - Reopen", "Ohio - December 8",
                                  "Indiana - Reopen", "Indiana - December 8", 
                                  "Missouri - Reopen", "Missouri - December 8", 
                                  "South Carolina - Reopen", "South Carolina - December 8"))
  # map
  # library(ggplot2)
  # library(cowplot)
  # library(colorspace)
  ...
  p3 <- ggplot(data = filter(DT_both, metric_type %in% "FPCA"), 
  ...
  return("figure3_CloseReopen.pdf")
