...
map_gif_fun <- function(res){
  ...
  DT_merged$state = factor(DT_merged$state, 
                         levels=c("Ohio", "Missouri", "South Carolina",
                                  "Indiana"))
  # library(ggplot2)
  # library(cowplot)
  # library(colorspace)
  ...
  DT_merged <- filter(DT_merged, date >= "2020-05-12")
  ...
  map_plot <- ggplot(data = DT_merged, 
    ...
    labs(title = "Date: {frame_time}", fill = "FPCA")
    
  mapGIF <- animate(map_plot, fps = 0.5, renderer = magick_renderer())
  
  anim_save("map.gif", mapGIF)
